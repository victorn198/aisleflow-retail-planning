...
echarts.use([EBarChart, ELineChart, GridComponent, MarkPointComponent, TooltipComponent, LegacyGridContainLabel, CanvasRenderer])
...
export function BarChart({ data, lang, onSelect }: { data: BreakdownRow[]; lang: Lang; onSelect?:(name:string)=>void }) {
  ...
  const previousMarkers=seriesData.flatMap((row,index)=>row.hasPrevious?[{coord:[row.previous,index],symbol:'rect',symbolSize:[3,18]}]:[])
  const currentSeries={name:lang==='pt'?'Período atual':'Current period',type:'bar',data:seriesData,barWidth:10,itemStyle:{color:'#0f766e',borderRadius:[0,4,4,0]},emphasis:{disabled:true},markPoint:{silent:true,label:{show:false},itemStyle:{color:'#657a76'},data:previousMarkers},z:2,label:{show:true,position:'right',formatter:label,rich:{value:{color:'#415269'},divider:{color:'#aab4c3'},up:{color:'#07855b',fontWeight:700},down:{color:'#d9364f',fontWeight:700},new:{color:'#1f6feb',fontWeight:700},neutral:{color:'#7c8798'}}}}
  const option={animationDuration:350,grid:{left:12,right:108,top:12,bottom:12,containLabel:true},tooltip:{trigger:'item',formatter:(p:{name:string}&LabelPoint)=>`${p.name}<br/>${format(p.data.value)} | ${comparison(p)}`},xAxis:{type:'value',splitNumber:3,max:share?100:undefined,splitLine:{lineStyle:{color:'#edf1f7'}},axisLabel:{color:'#697386',fontSize:10,hideOverlap:true,formatter:format}},yAxis:{type:'category',data:sorted.map(d=>d.name),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'#27364b',width:130,overflow:'truncate'}},series:[currentSeries]}
  return <><div className="chart-modes"><small className="comparison-key">{lang==='pt'?'Barra: atual · marca: anterior':'Bar: current · marker: prior'}</small><button className={!share?'active':''} onClick={()=>setShare(false)}>{lang==='pt'?'Valor':'Value'}</button><button className={share?'active':''} onClick={()=>setShare(true)}>{lang==='pt'?'Participação':'Share'}</button></div><div className="chart-viewport"><ReactEChartsCore echarts={echarts} option={option} onEvents={onSelect?{click:(params:{name:string})=>onSelect(params.name)}:undefined} style={{height:300,width:'100%'}}/></div></>
}
