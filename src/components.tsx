...
export function BarChart({ data, lang, onSelect }: { data: BreakdownRow[]; lang: Lang; onSelect?:(name:string)=>void }) {
  ...
  const priorSeries={name:lang==='pt'?'Período anterior':'Prior period',type:'bar',data:seriesData.map(row=>row.hasPrevious?row.previous:null),barWidth:18,itemStyle:{color:'rgba(15,118,110,.08)',borderColor:'#8aa9a4',borderWidth:1,borderRadius:[0,4,4,0]},silent:true,tooltip:{show:false},z:1}
  const currentSeries={name:lang==='pt'?'Período atual':'Current period',type:'bar',data:seriesData,barWidth:10,barGap:'-78%',itemStyle:{color:'#0f766e',borderRadius:[0,4,4,0]},emphasis:{disabled:true},z:2,label:{show:true,position:'right',formatter:label,rich:{value:{color:'#415269'},divider:{color:'#aab4c3'},up:{color:'#07855b',fontWeight:700},down:{color:'#d9364f',fontWeight:700},new:{color:'#1f6feb',fontWeight:700},neutral:{color:'#7c8798'}}}}
  ...
  return <><div className="chart-modes"><small className="comparison-key">{lang==='pt'?'Cheio: atual · contorno: anterior':'Filled: current · outline: prior'}</small><button className={!share?'active':''} onClick={()=>setShare(false)}>{lang==='pt'?'Valor':'Value'}</button><button className={share?'active':''} onClick={()=>setShare(true)}>{lang==='pt'?'Participação':'Share'}</button></div><div className="chart-viewport"><ReactEChartsCore echarts={echarts} option={option} onEvents={onSelect?{click:(params:{name:string})=>onSelect(params.name)}:undefined} style={{height:300,width:'100%'}}/></div></>
}
