...
export function BarChart({ data, lang, onSelect }: { data: BreakdownRow[]; lang: Lang; onSelect?:(name:string)=>void }) {
  ...
  const total=sorted.reduce((s,r)=>s+r.value,0),previousTotal=sorted.reduce((s,r)=>s+(r.previous??0),0),format=(v:number)=>share?`${v.toFixed(1)}%`:compact(v),seriesData=sorted.map(row=>{const value=share?(total?100*row.value/total:0):row.value,hasPrevious=row.previous!==undefined,previous=share?(previousTotal?100*(row.previous??0)/previousTotal:0):(row.previous??0),delta=previous?100*(value-previous)/Math.abs(previous):0;return{value,previous,delta,hasPrevious,isNew:hasPrevious&&previous===0}}),comparison=(p:LabelPoint,rich=false)=>!p.data.hasPrevious?(rich?`{neutral|${lang==='pt'?'sem anterior':'no prior'}}`:(lang==='pt'?'sem anterior':'no prior')):p.data.isNew?(rich?`{new|${lang==='pt'?'nova':'new'}}`:(lang==='pt'?'nova':'new')):rich?`{${p.data.delta>=0?'up':'down'}|${p.data.delta>=0?'▲':'▼'} ${Math.abs(p.data.delta).toFixed(1)}%}`:`${p.data.delta>=0?'▲':'▼'} ${Math.abs(p.data.delta).toFixed(1)}%`,label=(p:LabelPoint)=>`{value|${format(p.data.value)}} {divider||} ${comparison(p,true)}`
  const priorSeries={name:lang==='pt'?'Período anterior':'Prior period',type:'bar',data:seriesData.map(row=>row.hasPrevious?row.previous:null),barWidth:8,itemStyle:{color:'#c7d0dc',borderRadius:[0,4,4,0]},emphasis:{disabled:true}}
  const currentSeries={name:lang==='pt'?'Período atual':'Current period',type:'bar',data:seriesData,barWidth:10,itemStyle:{color:'#0f766e',borderRadius:[0,4,4,0]},emphasis:{disabled:true},z:2,label:{show:true,position:'right',formatter:label,rich:{value:{color:'#415269'},divider:{color:'#aab4c3'},up:{color:'#07855b',fontWeight:700},down:{color:'#d9364f',fontWeight:700},new:{color:'#1f6feb',fontWeight:700},neutral:{color:'#7c8798'}}}}
  const tooltip=(params:Array<{dataIndex:number}>)=>{const index=params[0]?.dataIndex??0,row=seriesData[index],name=sorted[index]?.name??'',color=row.delta>=0?'#07855b':'#d9364f';return `<b>${name}</b><br/>${lang==='pt'?'Atual':'Current'}: ${format(row.value)}<br/>${lang==='pt'?'Anterior':'Prior'}: ${row.hasPrevious?format(row.previous):'—'}<br/>${lang==='pt'?'Variação':'Change'}: <span style="color:${color};font-weight:700">${comparison({data:row})}</span>`}
  const option={animationDuration:350,grid:{left:12,right:108,top:12,bottom:12,containLabel:true},tooltip:{trigger:'axis',axisPointer:{type:'shadow'},formatter:tooltip},xAxis:{type:'value',splitNumber:3,max:share?100:undefined,splitLine:{lineStyle:{color:'#edf1f7'}},axisLabel:{color:'#697386',fontSize:10,hideOverlap:true,formatter:format}},yAxis:{type:'category',data:sorted.map(d=>d.name),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'#27364b',width:130,overflow:'truncate'}},series:[priorSeries,currentSeries]}
  return <><div className="chart-modes"><small className="comparison-key"><span><i className="current"/> {lang==='pt'?'Atual':'Current'}</span><span><i className="prior"/> {lang==='pt'?'Anterior':'Prior'}</span></small><button className={!share?'active':''} onClick={()=>setShare(false)}>{lang==='pt'?'Valor':'Value'}</button><button className={share?'active':''} onClick={()=>setShare(true)}>{lang==='pt'?'Participação':'Share'}</button></div><div className="chart-viewport"><ReactEChartsCore echarts={echarts} option={option} onEvents={onSelect?{click:(params:{name:string})=>onSelect(params.name)}:undefined} style={{height:300,width:'100%'}}/></div></>
}
