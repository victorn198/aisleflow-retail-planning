...
export default function App() {
  ...
  const periodOptions = [
    ['7', lang === 'pt' ? '7 dias' : '7 days'],
    ['30', lang === 'pt' ? '30 dias' : '30 days'],
    ['90', lang === 'pt' ? '90 dias' : '90 days'],
    ['all', lang === 'pt' ? 'Histórico' : 'History']
  ] as const
  ...
  return <div className="app-shell">
    ...
    <main id="top">
      ...
      <section className="filterbar"><div className="filter-heading"><span>{t.filters}</span><small>{data.meta.rows.toLocaleString()} rows</small></div><div className="period-control"><span>{lang==='pt'?'Período':'Period'}</span><div className="period-options">{periodOptions.map(([value,label])=><button key={value} className={filters.period===value?'active':''} disabled={queryState==='loading'} onClick={()=>{setDrillPath([]);setFilters(v=>({...v,period:value}))}}>{label}</button>)}</div></div><div className="comparison-contract"><span>{lang==='pt'?'Comparar vendas com':'Compare sales with'}</span><b>{filters.period==='all'?(lang==='pt'?'Sem baseline':'No baseline'):(lang==='pt'?'Período anterior equivalente':'Previous equivalent period')}</b></div>{select(t.channel, 'channel', data.filters.channels)}{select(t.device, 'device', data.filters.devices)}{select(t.country, 'country', data.filters.countries)}<button className="reset" onClick={() => {setDrillPath([]);setFilters({ channel: '', device: '', country: '',period:'30' })}}><RotateCcw size={15} />{t.reset}</button></section>
      ...
      {queryState==='ready'&&mart&&<><p className="scope-caption"><b>{mart.context.currentStart} — {mart.context.currentEnd}</b> · {comparisonAvailable ? `${lang==='pt'?'vendas comparadas com':'sales compared with'} ${filters.period} ${lang==='pt'?'dias anteriores':'prior days'}` : (lang==='pt'?'histórico completo, sem baseline':'complete history, no baseline')} · {lang==='pt'?'inventário: snapshot atual':'inventory: current snapshot'}</p><section className="metrics">{metrics.map(metric => {const isSnapshot=snapshotMetrics.has(metric.id);return <MetricCard key={metric.id} metric={metric} lang={lang} prior={isSnapshot?(lang==='pt'?'snapshot atual':'current snapshot'):comparisonLabel} comparisonAvailable={comparisonAvailable&&!isSnapshot} />})}</section>
      ...
      <PageInsight pageId={page.id} values={mart.current} trend={trend} breakdown={breakdown} detail={detail} forecast={forecast} lang={lang}/>
