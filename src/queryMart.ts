...
async function runQuery(filters:FilterState,pageId:string,drillPath:DrillItem[]):Promise<MartResult>{
 ...
 if(pageId==='forecast'){
  ...
  detailTable=await con.query(`${scoped} select 'Return or adjustment semantics' check_name,case when count(*) filter(where sales_amount<0 or units<0 or transactions<0 or cogs<0)=0 then 'Passed' else 'Review' end,(count(*) filter(where sales_amount<0 or units<0 or transactions<0 or cogs<0))::varchar observed from scoped union all select 'Required dimensions present',case when count(*) filter(where division is null or store is null or product_id is null or sale_date is null)=0 then 'Passed' else 'Review' end,(count(*) filter(where division is null or store is null or product_id is null or sale_date is null))::varchar from scoped union all select 'Sales and COGS available',case when count(*) filter(where sales_amount is null or cogs is null)=0 then 'Passed' else 'Review' end,(count(*) filter(where sales_amount is null or cogs is null))::varchar from scoped`)
 }else{
